        ...
        <div class="zc">
            <div class="bj_bai" id="regist-left" style="height: 468px">
                ...
                <form action="${ctx}/doRegister" method="post" id="registerForm">
                    ...
                    <input name="注册" type="button" class="btn_zhuce" id ="to_register" value="注册" onclick="checkForm()">
                    <%--<button name="注册" class="btn_zhuce" id ="to_register" value="注册">--%>
...
<script type="text/javascript">
    ...
    function checkPhone(){
        ...
        /*if(phone == ""){
            ...
            var hgt = $("#regist-left").height();
            if(v==0){
                $("#regist-left").height(hgt+20);
                $("#regist-right").height(hgt+20);
                v++;
            }
            flag2 =  false;
        }*/
        ...
        if(!(/^1[3|4|5|8|7][0-9]\d{8}$/.test(phone)))
        {
            ...
            //console.log("发送数据" + phone);
            /**
             * ajax一般发送的是json对象，而后台@RequestBody请求的是json字符串，
             * 所以要在发送前用JSON.stringify（）进行转换
             * @type {string}
             */
            var jsonPhone = JSON.stringify({"phone":phone});
            $.ajax({
                type:'post',
                url:'/checkPhone',
                data: jsonPhone,
    ...
    function checkEmail() {
       ...
       // alert(email);
        ...
        if(!(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+$/.test(email))){
            ...
            //验证邮箱是否已经注册
            var jsonEmail = JSON.stringify({"email":email});
            $.ajax({
                type:'post',
                url:'/checkEmail',
                data: jsonEmail,
    ...
    function checkCode() {
        ...
        if(code == ""){
            ...
            var jsonCode = JSON.stringify({"code":code});
            $.ajax({
                type: 'post',
                url: '/checkCode',
                data: jsonCode,
