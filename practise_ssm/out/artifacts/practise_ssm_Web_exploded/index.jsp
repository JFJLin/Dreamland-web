...
<div class="container">
    ...
    <div style="position: absolute;margin-left: 980px;margin-top: -40px;">
        ...
        <c:if test="${not empty user}">
            <a name="tj_loginp" href="javascript:void(0);"   class="lb" onclick="personal('${user.userid}');" style="color: black"><font color="#9370db">${user.username}, 欢迎您！</font></a>
            &nbsp;&nbsp;
    ...
    <nav class="navbar navbar-inverse">
        ...
        <div id="navbar-menu" class="collapse navbar-collapse">
            <ul class="nav navbar-nav">
                ...
                <li><a href="#">梦问答</a></li>
                <li><a href="#">我的梦</a></li>
                <li><a href="${ctx}/list?id=${user.userid}">个人空间</a></li>
            </ul>
        </div>
        ...
        <div class="col-md-9"  style="background-color: white;">
            <div id="content_col" class="content-main">
                ...
                <c:forEach var="cont" items="${page.result}" varStatus="i">
                   ...
                   <%-- <h2>结果: ${page.result}</h2>--%>
                    <div class="content-text">

                        <div class="author clearfix">
                            <div>
                                <a href="#" target="_blank" rel="nofollow" style="height: 35px">
                                     <img src="${cont.articleUserhead}">
                                </a>
                            </div>
                            <a href="#" target="_blank">
                                <h2 class="author-h2">
                                        ${cont.articleUsername}
                                </h2>
                            </a>
                        </div>

                        <h2>${cont.articeTitle}</h2>
                            ${cont.articleContent}
                        <div style="height: 5px"></div>
                        <div class="stats">
                            <!-- 笑脸、评论数等 -->
                            <span class="stats-vote"><i id="${cont.articleid}" class="number">${cont.articleUpvote}</i> 赞</span>
                            <span class="stats-comments">
                    <span class="dash"> · </span>
                         <a  onclick="reply(${cont.articleid},${cont.articleUserid})">
                              <i class="number" id="comment_num_${cont.articleid}">${cont.articleComment}</i> 评论
                          </a>
                    </span>
                        </div>
                        <div style="height: 5px"></div>
                        ...
                        <div class="stats-buttons bar clearfix">
                                    ...
                                    <%--点赞a标签--%>
                            <a style="cursor: pointer;" onclick="upvote_click(${cont.articleid},1)">
                                <i class="icon icon-thumbs-o-up icon-2x"></i>
                                <span class="number hidden" id="up_${cont.articleid}">${cont.articleUpvote}</span>
                            </a>
                            &nbsp;
                                    <%--踩a标签--%>
                            <a style="cursor: pointer;" onclick="upvote_click(${cont.articleid},-1)">
                                <i class="icon icon-thumbs-o-down icon-2x"></i>
                                <span class="number hidden" id="down_${cont.articleid}">${cont.articleCai}</span>
                            </a>
                            &nbsp;

                                    <%--评论a标签--%>
                            <a style="cursor: pointer;" onclick="reply(${cont.articleid},${cont.articleUserid})" title="点击打开或关闭">
                        ...
                        <br/>
                        &nbsp;
                        <div class="commentAll" style="display:none" id="comment_reply_${cont.articleid}">
                            <!--评论区域 begin-->
                            <div class="reviewArea clearfix">
                                    <textarea id="comment_input_${cont.articleid}" class="content comment-input" placeholder="请输入评论内容...&hellip;" onkeyup="keyUP(this)"></textarea>
                                <a class="plBtn" id="comment_${cont.articleid}" onclick="_comment(${cont.articleid},${user.userid==null?0:user.userid},${cont.articleUserid})" style="color: white;cursor: pointer;">评论</a>
                            </div>
                            <!--评论区域 end-->
                            <div class="comment-show-first" id="comment-show-${cont.articleid}">
            ...
            <div id="page-info" style="position: absolute;width:900px;background-color: #EBEBEB;height: 80px;left: 0px;">
                <ul class="pager pager-loose">
                    ...
                    <c:if test="${page.pageNum > 1}">
                        <li class="previous"><a href="${ctx}/index_list?pageNum=${page.pageNum-1}&&id=${user.userid}">« 上一页</a></li>
                    </c:if>
                    <c:forEach begin="1" end="${page.pages}" var="pn">
                        ...
                        <c:if test="${page.pageNum!=pn}">
                            <li ><a href="${ctx}/index_list?pageNum=${pn}&&id=${user.userid}">${pn}</a></li>
                        </c:if>
                    </c:forEach>
                    ...
                    <c:if test="${page.pageNum<page.pages}">
                        <li><a href="${ctx}/index_list?pageNum=${page.pageNum+1}&&id=${user.userid}">下一页 »</a></li>
                    </c:if>
...
<script language=javascript>
    ...
    function upvote_click(id,cont) {
        var jsonData = JSON.stringify({"id":id,"uid":'${user.userid}',"upvote":cont});
        $.ajax({
            type: 'post',
            url: '/upvote/upvotes',
            data: jsonData,
    ...
    function reply(id,uid) {
        ...
        if(showdiv_display = document.getElementById('comment_reply_'+id).style.display=='none'){//如果show是隐藏的
            ...
            var jsonData = JSON.stringify({"content_id":id});
            $.ajax({
                type:'post',
                url:'/reply',
                data: jsonData,
                        ...
                        $(list).each(function () {
                          //  console.log(this);
                            var chtml = "<div class='comment-show'>"+
                                ...
                                "<div class='comment-show-con-img pull-left'><img src='"+this.user.userhead+"' alt=''></div>"+
                                "<div class='comment-show-con-list pull-left clearfix'>"+
                                "<div class='pl-text clearfix'>"+
                                "<a  class='comment-size-name'>"+this.user.username+" :</a>"+
                                ...
                                "<a onclick='deleteComments("+id+","+uid+","+this.commentId+",null)' id='comment_dl_"+this.id+"' style='cursor:pointer' class='date-dz-pl pl-hf hf-con-block pull-left'>删除</a>"+
                                /*"<a style='cursor:pointer' onclick='comment_hf("+id+","+this.commentId+","+null+","+this.user.userid+","+uid+")' id='comment_hf_"+this.commentId+"' class='date-dz-pl pl-hf hf-con-block pull-left'>回复</a>"+*/
                                "<span class='pull-left date-dz-line'>|</span>"+
                                "<a onclick='reply_up("+this.commentId+")' style='cursor:pointer' class='date-dz-z pull-left' id='change_color_"+this.commentId+"'><i class='date-dz-z-click-red'></i>赞 (<i class='z-num' id='comment_upvote_"+this.commentId+"'>"+this.commentUpvotenum+"</i>)</a>"+
                                "</div></div> <div class='hf-list-con' id='hf-list-con-"+this.commentId+"'>";


                            var ehtml =   "</div> </div> </div></div>";
                            var parentComm_id = this.commentId;

                            okHtml = chtml;
                           // alert(parentComm_id)
                           // alert(this.commentSondisscusid);
                            if(this.commentSondisscusid != null && this.commentSondisscusid != ''){
                                var commentList = this.comList;
                                console.log(commentList);
                                $(commentList).each(function () {
                                    // alert(this.id);
                                    var oHtml = "<div class='all-pl-con'><div class='pl-text hfpl-text clearfix'>"+
                                        "<a class='comment-size-name'>"+this.user.username+"<a class='atName'>@"+this.byUser.username+" :</a> </a>"+
                                        "<span class='my-pl-con'>"+this.commentContent+"</span>"+
                                        "</div><div class='date-dz'> <span class='date-dz-left pull-left comment-time'>"+FormatDate(this.commentTime)+"</span>"+
                                        "<div class='date-dz-right pull-right comment-pl-block'>"+
                                        "<a style='cursor:pointer' onclick='deleteComments("+id+","+uid+","+this.commentId+","+parentComm_id+")' id='comment_dl_"+this.id+"' class='removeBlock'>删除</a>"+
                                        "<a onclick='comment_hf("+id+","+this.commentId+","+parentComm_id+","+this.user.userid+","+uid+")' id='comment_hf_"+this.commentId+"' style='cursor:pointer' class='date-dz-pl pl-hf hf-con-block pull-left'>回复</a> <span class='pull-left date-dz-line'>|</span>"+
                                        "<a onclick='reply_up("+this.commentId+")' id='change_color_"+this.commentId+"' style='cursor:pointer' class='date-dz-z pull-left'><i class='date-dz-z-click-red'></i>赞 (<i class='z-num' id='comment_upvote_"+this.commentId+"'>"+this.commentUpvotenum+"</i>)</a>"+
                                        "</div></div> </div>";
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(comm_data == "fail"){
                        ...
                        var id = comm_data.commentId;
                        oHtml = '<div class="comment-show-con clearfix"><div class="comment-show-con-img pull-left"><img src="${use.userhead}" alt=""></div> <div class="comment-show-con-list pull-left clearfix"><div class="pl-text clearfix"> <a  class="comment-size-name">${user.username} : </a> <span class="my-pl-con">&nbsp;'+ oSize +'</span> </div> <div class="date-dz"> <span class="date-dz-left pull-left comment-time">'+now+'</span> <div class="date-dz-right pull-right comment-pl-block"><a style="cursor:pointer"  onclick="deleteComment('+content_id+','+cuid+','+id+','+null+')" id="comment_dl_'+id+'"  class="date-dz-pl pl-hf hf-con-block pull-left">删除</a> <span class="pull-left date-dz-line">|</span> <a onclick="reply_up('+id+')" id="change_color_'+id+'" style="cursor:pointer"  class="date-dz-z pull-left" ><i class="date-dz-z-click-red"></i>赞 (<i class="z-num" id="comment_upvote_'+id+'">0</i>)</a> </div> </div><div class="hf-list-con"></div></div> </div>';
    ...
    //删除评论块
    function deleteComments(con_id,uid,id,fid) {
    ...
    function comment_pl(content_id,comment_id,fid,by_id,cuid) {
        ...
        if(oHfVal.replace(/^ +| +$/g,'') == '' || oHfVal == oAllVal){
            ...
            $.ajax({
                type:'post',
                url:'/comment_child',
                data: {"content_id":content_id,"uid":'${user.userid}',"oSize":oHfVal,"comment_time":now,"by_id":by_id,"id":fid},
                dataType:'json',
                success:function(data){
                    var comm_data =  data["data"];
                    //alert(comm_data);
                    if(comm_data=="fail"){
                        ...
                        var oHtml = '<div class="all-pl-con"><div class="pl-text hfpl-text clearfix"><a class="comment-size-name">${user.username} : </a><span class="my-pl-con">'+oAt+'</span></div><div class="date-dz"> <span class="date-dz-left pull-left comment-time">'+now+'</span> <div class="date-dz-right pull-right comment-pl-block"> <a style="cursor:pointer" onclick="deleteComment('+content_id+','+cuid+','+id+','+fid+')" id="comment_dl_'+id+'" class="removeBlock">删除</a> <a onclick="comment_hf('+content_id+','+id+','+fid+','+comm_data.user.userid+','+cuid+')" id="comment_hf_'+id+'" style="cursor:pointer" class="date-dz-pl pl-hf hf-con-block pull-left">回复</a> <span class="pull-left date-dz-line">|</span> <a onclick="reply_up('+id+')" id="change_color_'+id+'" style="cursor:pointer" class="date-dz-z pull-left"><i class="date-dz-z-click-red"></i>赞 (<i class="z-num" id="comment_upvote_'+id+'">0</i>)</a> </div> </div></div>';
